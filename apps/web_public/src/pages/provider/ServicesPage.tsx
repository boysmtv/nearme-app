          ...
          <div key={s.id} className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3"><h3 className="font-semibold text-gray-900">{s.name}</h3><span className={`rounded-full px-2.5 py-1 text-sm font-medium ${s.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{s.active ? 'Aktif' : 'Nonaktif'}</span></div>
                <p className="mt-1 text-sm text-gray-500">{s.description}</p>
                <div className="mt-2 flex items-center gap-4 text-sm text-gray-500"><span>{s.duration} menit</span><span>{fmt(s.price)}</span></div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => { setEdit(s); reset({ name: s.name, description: s.description, duration: s.duration, price: s.price }); setShow(true); }} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Edit</button>
                <button onClick={() => { setAddonsFor(s.id); setAddons((s as unknown as { addons?: { name: string; price: number; duration: number }[] }).addons ?? []); }} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Add-on</button>
                <button onClick={() => { if (confirm('Hapus layanan ini?')) deleteMut.mutate(s.id); }} className="rounded-lg border border-red-200 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50">Hapus</button>
              </div>
              {addonsFor === s.id && (
                <div className="mt-4 rounded-lg border border-gray-200 p-3 bg-gray-50">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <input value={addonName} onChange={(e)=> setAddonName(e.target.value)} placeholder="Nama add-on" className="flex-1 min-w-[120px] rounded border border-gray-300 px-3 py-2 text-sm h-9" />
                    <input type="number" value={addonPrice} onChange={(e)=> setAddonPrice(parseInt(e.target.value)||0)} placeholder="Harga" className="w-24 rounded border border-gray-300 px-3 py-2 text-sm h-9" />
                    <input type="number" value={addonDur} onChange={(e)=> setAddonDur(parseInt(e.target.value)||15)} placeholder="Dur (min)" className="w-20 rounded border border-gray-300 px-3 py-2 text-sm h-9" />
                    <button onClick={() => { if (!addonName.trim()) return; setAddons(prev=> [...prev, { name: addonName.trim(), price: addonPrice, duration: addonDur }]); setAddonName(''); setAddonPrice(0); }} className="rounded bg-primary-600 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-700">+ Tambah</button>
                  </div>
                  <div className="mt-3 flex gap-2">
                    <button onClick={() => saveAddonsMut.mutate()} disabled={saveAddonsMut.isPending} className="rounded bg-primary-600 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-700 disabled:opacity-50">{saveAddonsMut.isPending ? 'Menyimpan...' : 'Simpan Add-ons'}</button>
                    <button onClick={() => { setAddonsFor(null); setAddons([]); }} className="rounded border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-white">Batal</button>
                  </div>
                  {saveAddonsMut.isSuccess && <p className="mt-2 text-sm text-green-600">Add-ons tersimpan</p>}
                  {saveAddonsMut.isError && <p className="mt-2 text-sm text-red-600">{(saveAddonsMut.error as Error).message}</p>}
                </div>
              )}
            </div>
          </div>
