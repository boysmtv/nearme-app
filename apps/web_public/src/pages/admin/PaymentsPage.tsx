...
export default function PaymentsPage() {
  ...
  const { data: res, isLoading } = useQuery({ queryKey: ['admin', 'payments', page, filterStatus], queryFn: () => adminApi.payments.list({ page, limit: 20, status: filterStatus }) });
  ...
  return (<AdminLayout><div className="space-y-6"><div><h1 className="text-2xl font-bold text-gray-900">Payments</h1><p className="mt-1 text-sm text-gray-500">Rekonsiliasi dan status pembayaran</p></div><select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setPage(1); }} className="rounded-lg border border-gray-300 px-3 py-2 text-sm"><option value="">Semua Status</option>{['PENDING','PAID','FAILED','REFUNDED','EXPIRED','CANCELLED'].map((s) => <option key={s} value={s}>{s}</option>)}</select><DataTable columns={columns} data={res?.data?.data ?? []} pagination={res?.data?.pagination} onPageChange={setPage} isLoading={isLoading} emptyMessage="Tidak ada pembayaran" /></div></AdminLayout>);
}
