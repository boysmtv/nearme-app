...
class AuthState extends Equatable {
  ...
  /// Role untuk routing shell di aplikasi unifikasi.
  /// Default ROLE_CUSTOMER bila JWT tidak membawa role (backward compat).
  String get role => user?.role ?? 'ROLE_CUSTOMER';
  bool get isProvider => role.startsWith('ROLE_PROVIDER');
  bool get isAdmin => role == 'ROLE_PLATFORM_ADMIN';
}
...
User? userFromAccessToken(String accessToken, String? fallbackEmail) {
  try {
    ...
    // Backend mengirim claim `roles: ["ROLE_..."]`; ambil role pertama untuk
    // routing shell (customer / provider / admin) di aplikasi unifikasi.
    String? role;
    ...
    return User(id: id, email: email ?? '', name: email?.split('@').first, role: role);
  } catch (_) {
    return null;
  }
}
