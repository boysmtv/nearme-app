...
- **Payment gateway integration**: Midtrans/Xendit gateway adapters are built and wired. Need to configure real API keys in `application-dev.yml` (`dekat.payment.gateway.serverKey`) and test end-to-end with actual gateway sandbox
- **Mobile partner app device testing**: App compiles (43 tests pass), `google-services.json` dummy added, `POST_NOTIFICATIONS` permission added. Need to run on Mi A1 to verify FCM, login, calendar, booking management
- **Booking PIN device testing**: Backend endpoints exist, web UI wired (BookingPage + CustomerBookingDetailPage), mobile UI wired (booking_detail_page). Need to test full flow on device: create booking → get PIN → verify PIN → status updates
- **Customer dashboard page**: Consider adding summary page with recent bookings, loyalty points, quick actions
