...
- **Availability FIXED 2026-08-29** — `availability_page.dart:11` `String key` `'$providerId|$date'` fix infinite loop (Map identity bug → 70ms loop), layout `availability_page.dart:39` card calendar `radius 20` shadow, service dropdown card, `Available Times` grouped Morning/Afternoon/Evening `Wrap` chip `AnimatedContainer` primary selected, shimmer loading, `usesCleartextTraffic` di `AndroidManifest.xml:4` untuk `http://192.168.100.55:8080` di Android 9
 ...
 - **Customer dashboard + all pending completed 2026-09-09** — (1) **Payment gateway verified**: `MidtransGatewayAdapter` + `PaymentGatewayProperties` fully wired, sandbox URL configured (`https://api.sandbox.midtrans.com`), real API keys needed from user. (2) **Both Flutter apps build**: `mobile_customer` and `mobile_partner` APKs compile successfully. (3) **All 350 tests green**: backend BUILD SUCCESSFUL + 134 web_public + 49 web_admin + 124 mobile_customer + 43 mobile_partner. (4) **Customer dashboard page created**: `CustomerDashboardPage.tsx` with welcome message, quick actions (Search/Bookings/Favorites/Notifications), loyalty points card (Bronze/Silver/Gold tier), recent bookings list, and 6 shortcut links. Route `/` shows dashboard for `ROLE_CUSTOMER`, public HomePage for guests.
  ...
  - **Auth back-button fix 2026-09-18** — Guest diarahkan ke `/login` via GoRouter `redirect` yang bersifat *replace* (back stack hilang) + tab proteksi pakai `go` (bukan `push`), sehingga back Android langsung menutup aplikasi; `pop()` mentah bahkan bisa loop (kembali ke halaman proteksi → redirect → login lagi). Fix: `AuthBackGuard` (PopScope) di login/register/forgot — register/forgot `pop()` bila ada halaman di bawah (aman), selain itu ke target `?redirect=` bila publik, sisanya `go('/discovery')`; tidak pernah menutup aplikasi. Tombol back AppBar register/forgot + "Back to Login" disamakan perilakunya. 8 test baru (`isPublicLocation` + system back via `handlePopRoute`). Customer 106/106 hijau.
  - **Home jank fix 2026-09-18 (Mi A1)** — Struktur sudah full MVVM + Clean Architecture (domain/entities+usecases+repositories, data, presentation/viewmodel per 12 fitur — bukan penyebab berat). Biang berat: (1) `TweenAnimationBuilder` Opacity+Transform di tiap kartu provider & chip kategori (20+ layer komposit yang restart tiap rebuild + menganimasi kartu baru saat scroll 0.3-1.2s), (2) shimmer lokal 1 `AnimationController.repeat()` per item (~10 ticker + rebuild/frame saat loading). Fix: animasi per-item dihapus (header 1x dipertahankan), shimmer kategori jadi 1 `Shimmer.fromColors` shader, kartu jadi placeholder statis. Customer 106/106 hijau.
  - **Shopme reference audit 2026-09-18 (read-only)** — Bandingkan dengan `shopme_flutter` (konten lebih padat tapi enteng). Temuan yang diadopsi: (1) `RepaintBoundary` per kartu/item list (shopme: tiap section+item dibungkus; scroll tidak repaint sibling, shadow ter-cache di layer), (2) image di-decode secukupnya via `memCacheWidth/Height` 2x ukuran tampil (shopme: shared CacheManager + 2x; foto full-res → decode penuh = boros memori + raster berat), (3) tanpa animasi entry per-item (shopme cuma transisi route + 1 fade generik). Tidak diadopsi (sudah setara): startup post-frame (kita BootstrapGate), font bundled, shimmer per-section, ValueNotifier vs Riverpod (setara). Customer 106/106 hijau.
  - **Auth rebuild-storm fix 2026-09-18 (semua halaman berat)** — `AuthState` tanpa equality + tiap 401 saat guest memanggil `logout()` → notify → rebuild seluruh router berulang; endpoint authed (`/customer/profile`, `/notifications` badge) ditembak walau guest (pasti 401). Fix: `User` value `==`/`hashCode`, `AuthState extends Equatable`, `logout()` short-circuit bila sudah `const AuthState()`, provider badge+dashboard-profile return lokal bila guest (0/`{}`). Customer 112/112 hijau.
  - **Booking 400 fix 2026-09-18 ("Start time must be before end time")** — Root cause: API publik kirim key `duration`, tapi `ServiceEntity.fromJson` hanya baca `durationMinutes` → durasi selalu 0 → `endsAt == startsAt` → backend 400 di SEMUA booking (repro via curl: endsAt==startsAt → 400 persis, endsAt normal → hold 201). Fix: parser terima kedua key; hapus duplikat `bookingSummaryProvider`/`BookingSummary` di form (pakai versi viewmodel: parser benar + `Future.wait`); guard client-side tolak durasi ≤ 0 dengan pesan jelas. Customer 114/114 hijau.
  - **Payment 500 fix 2026-09-18 ("internal services error")** — Booking sukses tapi auto-intent `method='cash'` ditolak CHECK `payment_intents_method_check` (hanya CASH/CARD/BANK_TRANSFER/E_WALLET/QRIS/VIRTUAL_ACCOUNT) → 500. Fix 3 sisi: backend `normalizePaymentMethod` (alias case-insensitive, unknown → 400 jelas, null = pilih di gateway) + auto-deposit intent pakai null (bukan "DEPOSIT"); mobile kirim kanonis (`E_WALLET`/`BANK_TRANSFER`/`CARD`/`CASH`); web tidak lagi kirim `'midtrans'` (omit → pilih di gateway). Test backend `"midtrans"` → `"QRIS"` + 2 test normalisasi baru. Verified live: `cash` → `CASH` 201, `midtrans` → 400 jelas. Backend di-rebuild + redeploy (port 8081 karena 8080 dipakai project lain).
