...
### Testing
- **Total: ~405 tests** across 4 platforms, all passing (222 backend + 134 web_public + 49 web_admin + e2e 41)
...
- **Profile update bug**: `PUT /customer/profile` works from CLI (200 OK) but fails from frontend — need to check browser console for actual error message (updated `onError` to show real error + `console.error`)
- **Customer dashboard page**: Currently no dedicated dashboard — customers land on `/` (search/discovery). Consider adding summary page with recent bookings, loyalty points, quick actions
- **Complete profile flow**: After `PUT /customer/profile` succeeds, `completeProfile()` in `auth.tsx` sets `has_profile=true` in localStorage + rebuilds user — verify this works end-to-end
- **Mobile partner app**: `mobile_partner` app not yet tested on device — needs same `google-services.json` dummy + run on Mi A1
- **Booking payment flow**: Hold → Confirm works with `cash` method. Need to test Midtrans/Xendit payment gateway integration end-to-end on mobile
- **Booking PIN verification**: `POST /bookings/{id}/verify-pin` endpoint exists, mobile UI wired — test full flow on device
